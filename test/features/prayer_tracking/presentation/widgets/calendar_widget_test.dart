...
void main() {
  ...
  group('CalendarWidget', () {
    testWidgets(
      'renders correctly and shows Gregorian by default in collapsed header',
      (WidgetTester tester) async {
        final testDate = DateTime(2026, 3, 3); // March 3, 2026

        await tester.pumpWidget(
          createWidgetUnderTest(selectedDate: testDate, monthRecords: {}),
        );
    ...
    testWidgets('switches focus to Hijri when header switch button is tapped', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('renders Arabic correctly', (WidgetTester tester) async {
      ...
      expect(
        find.byWidgetPredicate(
          (widget) =>
              widget is Text && (widget.data?.contains('رمضان') ?? false),
        ),
        findsAtLeast(1),
      );
    });
