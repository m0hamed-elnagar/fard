...
void main() {
  group('ReadingSegment - Range Tracking Verification', () {
    ...
    test(
      'BUG DEMONSTRATION: Current buggy behavior creates single ayah segment',
      () {
    ...
    test('Segment merging should preserve ranges', () {
      ...
      for (final seg in merged) {
        print(
          '   Segment: ${seg.startAyah} - ${seg.endAyah} (${seg.ayahsCount} ayahs)',
        );
      }
    ...
    test('Calculate total ayahs from multiple segments', () {
      ...
      final totalAyahs = segments.fold<int>(
        0,
    ...
    test('WerdProgress should calculate total from segments', () {
      ...
      final calculatedTotal = progress.segmentsToday.fold<int>(
        0,
        (sum, seg) => sum + seg.ayahsCount,
      );
