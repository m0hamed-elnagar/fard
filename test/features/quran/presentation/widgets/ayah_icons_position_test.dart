...
void main() {
  ...
  testWidgets('Check visual order of Ayah Text, Marker and Green Symbols', (
    WidgetTester tester,
  ) async {
    ...
    final ayahTextBox = renderParagraph
        .getBoxesForSelection(
          const TextSelection(baseOffset: 1, extentOffset: 4),
        )
        .first;
    final ayahDx = renderParagraph
        .localToGlobal(ayahTextBox.toRect().center)
        .dx;
    ...
    final markerBox = renderParagraph
        .getBoxesForSelection(
          TextSelection(
            baseOffset: markerIndex,
            extentOffset: markerIndex + markerText.length,
          ),
        )
        .first;
    final markerDx = renderParagraph
        .localToGlobal(markerBox.toRect().center)
        .dx;

    // 3. Last Read Symbol position ➤
    final lastReadIndex = plainText.indexOf('\u27A4');
    expect(lastReadIndex, isNot(-1));
    final lastReadBox = renderParagraph
        .getBoxesForSelection(
          TextSelection(
            baseOffset: lastReadIndex,
            extentOffset: lastReadIndex + 1,
          ),
        )
        .first;
    final lastReadDx = renderParagraph
        .localToGlobal(lastReadBox.toRect().center)
        .dx;
    ...
    final bookmarkBox = renderParagraph
        .getBoxesForSelection(
          TextSelection(
            baseOffset: bookmarkIndex,
            extentOffset: bookmarkIndex + 2,
          ),
        )
        .first;
    final bookmarkDx = renderParagraph
        .localToGlobal(bookmarkBox.toRect().center)
        .dx;
    ...
    expect(
      ayahDx,
      greaterThan(markerDx),
      reason: 'Ayah text should be to the right of Marker',
    );
    expect(
      markerDx,
      greaterThan(lastReadDx),
      reason: 'Marker should be to the right of LastRead symbol',
    );
    expect(
      lastReadDx,
      greaterThan(bookmarkDx),
      reason: 'LastRead should be to the right of Bookmark symbol',
    );
  });
}
