...
void main() {
  ...
  setUp(() {
    ...
    bookmarkStreamController = StreamController<Result<List<Bookmark>>>.broadcast();
    ...
    when(() => mockBookmarkRepository.watchBookmarks()).thenAnswer((_) => bookmarkStreamController.stream);
    when(() => mockBookmarkRepository.getBookmarks()).thenAnswer((_) async => Result.success(List.from(currentBookmarks)));
    when(() => mockBookmarkRepository.isBookmarked(any())).thenAnswer((event) async {
      final ayahNum = event.positionalArguments[0] as AyahNumber;
      return Result.success(currentBookmarks.any((b) => b.ayahNumber == ayahNum));
    });
    
    when(() => mockBookmarkRepository.addBookmark(any())).thenAnswer((event) async {
    ...
    when(() => mockBookmarkRepository.removeBookmark(any())).thenAnswer((event) async {
      final ayahNum = event.positionalArguments[0] as AyahNumber;
    ...
    when(() => mockBookmarkRepository.clearAllBookmarks()).thenAnswer((_) async {
      currentBookmarks.clear();
  ...
  group('Decoupling Bookmark and Last Read', () {
    blocTest<ReaderBloc, ReaderState>(
      ...
      verify: (bloc) {
        ...
        expect(state?.lastReadAyah, isNull, reason: 'Bookmark should not set lastReadAyah');
      },
    );
    ...
    blocTest<ReaderBloc, ReaderState>(
      ...
      act: (bloc) async {
        ...
        bookmarkStreamController.add(Result.success([
          Bookmark(id: '1_2', ayahNumber: tAyah2.number, createdAt: DateTime.now())
        ]));
        await Future.delayed(const Duration(milliseconds: 50));
      },
      verify: (bloc) {
        final state = bloc.state.mapOrNull(loaded: (s) => s);
        expect(state?.bookmarks.any((b) => b.ayahNumber == tAyah2.number), isTrue);
        expect(state?.lastReadAyah, tAyah1, reason: 'Bookmark should not change existing lastReadAyah');
      },
    );
    ...
    blocTest<ReaderBloc, ReaderState>(
      ...
      verify: (bloc) {
        ...
        expect(state?.bookmarks, isEmpty, reason: 'Saving last read should not set bookmark');
      },
    );
    ...
    blocTest<ReaderBloc, ReaderState>(
      ...
      verify: (bloc) {
        ...
        expect(state?.bookmarks.any((b) => b.ayahNumber == tAyah1.number), isTrue);
        expect(state?.bookmarks.any((b) => b.ayahNumber == tAyah2.number), isTrue);
      },
    );
   group('Reactive Updates', () {
      blocTest<ReaderBloc, ReaderState>(
        ...
        act: (bloc) async {
          ...
          final externalBookmark = Bookmark(id: '1_1', ayahNumber: tAyah1.number, createdAt: DateTime.now());
