...
void main() {
  ...
  test('UpdateLastRead should NOT change sessionStartAbsolute if it is already set', () async {
    ...
    when(() => mockWerdRepository.getProgress(goalId: any(named: 'goalId')))
        .thenAnswer((_) async => Result.success(initialProgress));
    when(() => mockQuranRepository.updateLastReadPosition(any()))
    ...
    final pos5 = LastReadPosition(
      ayahNumber: AyahNumber.create(surahNumber: 1, ayahNumberInSurah: 5).data!,
      updatedAt: DateTime.now(),
    );
    await useCase(pos5);
  ...
  test('UpdateLastRead should NOT change sessionStartAbsolute even on a far jump', () async {
    ...
    final pos50 = LastReadPosition(
      ayahNumber: AyahNumber.create(surahNumber: 1, ayahNumberInSurah: 50).data!,
      updatedAt: DateTime.now(),
    );
    await useCase(pos50);
  ...
  test('UpdateLastRead should set sessionStartAbsolute if it is null (first read of day)', () async {
    ...
    final pos101 = LastReadPosition(
      ayahNumber: AyahNumber.create(surahNumber: 2, ayahNumberInSurah: 94).data!, // Surah 2, Ayah 94 is abs 101
      updatedAt: DateTime.now(),
    );
    ...
    verify(() => mockWerdRepository.updateProgress(any(
      that: isA<WerdProgress>()
          .having((p) => p.sessionStartAbsolute, 'sessionStart', 101),
    ))).called(1);
  });
}
