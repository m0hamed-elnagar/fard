...
void main() {
  ...
  const MethodChannel compassChannel = MethodChannel(
    'hemanthraj/flutter_compass',
  );
  binding.defaultBinaryMessenger.setMockMethodCallHandler(compassChannel, (
  ...
  const MethodChannel audioMethodsChannel = MethodChannel(
    'com.ryanheise.just_audio.methods',
  );

  binding.defaultBinaryMessenger.setMockMethodCallHandler(audioMethodsChannel, (
    MethodCall methodCall,
  ) async {
    if (methodCall.method == 'init') {
      ...
      binding.defaultBinaryMessenger.setMockMethodCallHandler(playerChannel, (
        ...
        switch (playerMethodCall.method) {
          case 'load':
            ...
            final ByteData eventData = const StandardMethodCodec()
      ...
      binding.defaultBinaryMessenger.setMockMessageHandler(
        'com.ryanheise.just_audio.data.$id',
        (message) async {
          return const StandardMethodCodec().encodeSuccessEnvelope(null);
        },
      );
  ...
  group('Quran Reader Flow Integration Test', () {
    testWidgets(
      'Full flow: navigate to Quran, verify big surah load, tafsir and audio',
      (tester) async {
        // 1. Setup environment
        ...
        try {
          ...
          expect(
            pauseButtonFinder.evaluate().isNotEmpty ||
                loadingIndicatorFinder.evaluate().isNotEmpty,
            isTrue,
        ...
        if (closeButtonFinder.evaluate().isNotEmpty) {
          ...
          debugPrint(
            'Close button not found, using pageBack() to close modal...',
          );
