...
void main() {
  ...
  group('Onboarding Azan Integration Test', () {
    ...
    setUp(() async {
      ...
      when(() => mockVoiceService.downloadAzan(any())).thenAnswer((_) async => '/mock/path/azan.mp3');
      when(() => mockNotificationService.canScheduleExactNotifications()).thenAnswer((_) async => true);
      when(() => mockNotificationService.testAzan(any(), any(), settings: any(named: 'settings'))).thenAnswer((_) async {});
    ...
    testWidgets('Verify 5-page onboarding flow including Azan selection', (tester) async {
      ...
      await tester.pump(const Duration(milliseconds: 500));
      await tester.pumpAndSettle();
      verify(() => mockNotificationService.testAzan(Salaah.fajr, any(), settings: any(named: 'settings'))).called(1);
