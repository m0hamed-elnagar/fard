...
void main() {
  ...
  group('Missed Days Flow Integration Test', () {
    ...
    testWidgets('Should show missed days dialog and handle "I was praying (All)"', (tester) async {
      ...
      if (boxCheck.isNotEmpty) {
        ...
        debugPrint('TEST: Record in box: ${r?.id}, dateMillis: ${r?.dateMillis}');
        final diff = DateTime.now().difference(DateTime.fromMillisecondsSinceEpoch(r!.dateMillis)).inDays;
      ...
      final key19 = '${date19.year}-${date19.month.toString().padLeft(2, '0')}-${date19.day.toString().padLeft(2, '0')}';
      final record19 = box.get(key19);
      expect(record19, isNotNull, reason: 'Record for 19th should exist (created as prayed)');
      expect(record19!.qadaValues[0], 10, reason: 'Fajr Qada on 19th should be 10');
      
      final date20 = threeDaysAgo.add(const Duration(days: 2));
      final key20 = '${date20.year}-${date20.month.toString().padLeft(2, '0')}-${date20.day.toString().padLeft(2, '0')}';
      final record20 = box.get(key20);
      expect(record20, isNotNull);
      expect(record20!.qadaValues[0], 10, reason: 'Fajr Qada on 20th should be 10');
    });

    testWidgets('Should toggle "Select All" and confirm manual selection', (tester) async {
      await tester.pumpWidget(app.QadaTrackerApp(hivePath: tempDir.path));
      ...
      expect(record19!.qadaValues[0], 11, reason: 'Fajr Qada on 19th should be 11 (10+1)');
      
      final date20 = threeDaysAgo.add(const Duration(days: 2));
      final key20 = '${date20.year}-${date20.month.toString().padLeft(2, '0')}-${date20.day.toString().padLeft(2, '0')}';
      final record20 = box.get(key20);
      expect(record20, isNotNull);
      expect(record20!.qadaValues[0], 12, reason: 'Fajr Qada on 20th should be 12 (11+1)');
    });
  });
}
