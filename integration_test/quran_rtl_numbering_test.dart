...
void main() {
  ...
  group('Quran RTL Numbering Integration Test', () {
    testWidgets(
      'Verify Ayah markers are ordered RTL (3 2 1 from left to right)',
      (tester) async {
        // 1. Setup environment
        ...
        final Finder surahTextFinder = find
            .descendant(
            ...
            .map((w) => find.byWidget(w))
            .first;

        expect(
          surahTextFinder,
          findsOneWidget,
          reason: 'AyahText should contain a RichText with markers',
        );
        ...
        double getMarkerGlobalDx(String marker) {
          ...
          final Offset globalCenter = renderParagraph.localToGlobal(
            localCenter,
          );
        ...
        expect(
          dx1,
          greaterThan(dx2),
          reason: 'Marker 1 should be to the right of Marker 2',
        );
        expect(
          dx2,
          greaterThan(dx3),
          reason: 'Marker 2 should be to the right of Marker 3',
        );
