...
class WerdTestHelpers {
  ...
  static Future<void> swipeToWerdCard(WidgetTester tester) async {
    ...
    expect(
      pageView,
      findsOneWidget,
      reason: 'PageView should exist in carousel',
    );
  ...
  static Future<void> markFirstAyahAsRead(WidgetTester tester) async {
    ...
    if (markBtn == null &&
        find.textContaining('Mark', findRichText: true).evaluate().isNotEmpty) {
  ...
  static Future<void> tapContinueButtonDirect(
    WidgetTester tester,
    int targetAyah,
  ) async {
  ...
  static void dumpWidgetTree(WidgetTester tester) {
    ...
    debugPrint(
      '🔘 Buttons: Elevated=$elevatedButtons, Text=$textButtons, Icon=$iconButtons',
    );
    ...
    try {
      ...
      if (progress != null) {
        ...
        for (var i = 0; i < progress.segmentsToday.length; i++) {
          ...
          debugPrint(
            '  Segment $i: ${seg.startAyah} → ${seg.endAyah} (${seg.ayahsCount} ayahs)',
          );
          debugPrint(
            '    Time: ${seg.formattedStartTime} - ${seg.formattedEndTime}',
          );
        }
      } else {
