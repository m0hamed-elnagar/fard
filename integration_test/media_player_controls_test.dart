...
void main() {
  ...
  group('Media Player Background and Controls Test', () {
    testWidgets('Verify Media Player Controls and State Persistence', (tester) async {
      ...
      expect(isActive, isTrue, reason: "Audio should be active (loading, playing, or paused)");
      ...
      if (audioBloc.state.status == AudioStatus.paused) {
        ...
        final playBtn = find.descendant(of: find.byType(AudioPlayerBar), matching: find.byIcon(Icons.play_arrow_rounded));
      ...
      if (audioBloc.state.isPlaying) {
        ...
        final pauseBtn = find.descendant(of: find.byType(AudioPlayerBar), matching: find.byIcon(Icons.pause_rounded));
      ...
      final resumeBtn = find.descendant(of: find.byType(AudioPlayerBar), matching: find.byIcon(Icons.play_arrow_rounded));
      ...
      final nextIcon = find.descendant(of: find.byType(AudioPlayerBar), matching: find.byIcon(Icons.skip_next_rounded));
      if (tester.any(nextIcon)) {
        await tester.tap(nextIcon.first);
        await tester.pumpAndSettle();
        await tester.pump(const Duration(seconds: 3)); // Wait for next track to load
        debugPrint('DEBUG: Initial Ayah: $initialAyah, Current Ayah: ${audioBloc.state.currentAyah}');
      }

      // 4. Test SKIP PREVIOUS (Previous Ayah)
      debugPrint('DEBUG: Testing Skip Previous');
      final prevIcon = find.descendant(of: find.byType(AudioPlayerBar), matching: find.byIcon(Icons.skip_previous_rounded));
      if (tester.any(prevIcon)) {
        await tester.tap(prevIcon.first);
        await tester.pumpAndSettle();
        await tester.pump(const Duration(seconds: 3));
        debugPrint('DEBUG: Current Ayah after skip back: ${audioBloc.state.currentAyah}');
      }
      ...
      expect(audioBloc.state.isPlaying, isTrue, reason: "Audio should continue playing when app is paused");
