...
class AudioDownloadServiceImpl implements AudioDownloadService {
  ...
  Future<void> downloadSurah({
    ...
    try {
      ...
      final entries = await _manifestService.getEntriesBySurah(
        reciterId,
        surahNumber,
      );
      if (entries.isEmpty) await _syncManifestForSurah(reciterId, surahNumber);

      final updatedEntries = await _manifestService.getEntriesBySurah(
        reciterId,
        surahNumber,
      );
      ...
      for (int i = 0; i < pendingEntries.length; i += batchSize) {
        ...
        if (_cancellationFlags[reciterId]?.contains(surahNumber) == true) {
          ...
          await FileDownloadUtils.pruneTempFiles(
            '${directory.path}/audio/$reciterId',
          );
          return;
        }
          ...
          batch.map((entry) async {
            ...
            while (retries > 0 && !success) {
              ...
              try {
                ...
                final directory = await getApplicationSupportDirectory();
                final finalPath =
                    '${directory.path}/audio/${entry.relativePath}';
      ...
      final finalEntries = await _manifestService.getEntriesBySurah(
        reciterId,
        surahNumber,
      );
      int verifiedCount = 0;
      ...
      _emitError(
        ...
        surahNumber,
        e.toString(),
        total: _audioRepository.getAyahCount(surahNumber),
      );
    }
  }
  ...
  Future<SurahDownloadStatus> getSurahStatus({
    ...
    final isStopping =
        isDownloading &&
        (_cancellationFlags[reciterId]?.contains(surahNumber) ?? false);
  ...
  Future<SurahDownloadStatus> _syncManifestForSurah(
    ...
    for (int i = 0; i < tracks.length; i++) {
      ...
      final actualSize = await FileDownloadUtils.getExistingFileSizeBytes(
        track.localPath,
      );
      final status = isValid
          ? DownloadStatus.completed
          : (actualSize > 0 ? DownloadStatus.paused : DownloadStatus.pending);
  ...
  Future<List<Reciter>> getRecitersWithDownloadedSurah(int surahNumber) async {
    ...
    for (final id in reciterIds) {
      final reciter = recitersResult.data!.firstWhere(
        (r) => r.identifier == id,
  ...
  Future<Set<int>> getDownloadedSurahIdsForReciter(String reciterId) async {
    ...
    for (final entry in entries) {
      ...
      if (entry.status == DownloadStatus.completed) {
        completeManifestAyahs
            .putIfAbsent(entry.surahNumber!, () => {})
            .add(entry.ayahNumber!);
      } else {
        incompleteManifestAyahs
            .putIfAbsent(entry.surahNumber!, () => {})
            .add(entry.ayahNumber!);
      }
    }
    ...
    for (int surahNum = 1; surahNum <= 114; surahNum++) {
      ...
      for (int ayahNum = 1; ayahNum <= expectedCount; ayahNum++) {
        final isCompleteInManifest =
            completeManifestAyahs[surahNum]?.contains(ayahNum) == true;
        final isIncompleteInManifest =
            incompleteManifestAyahs[surahNum]?.contains(ayahNum) == true;
        final existsOnDisk =
            fileSystemAyahs[surahNum]?.contains(ayahNum) == true;
